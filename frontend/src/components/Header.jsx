...
export default function Header() {
	...
	const handleSignIn = () => {
		window.location.href = "https://michalele23.live/auth/steam";
	};
	...
	const buttonStyles =
		"p-3 justify-center gap-3 w-16 xs:w-48 lg:w-64 rounded-md bg-secondary border-none";
	...
	let button = (
		<Button className={buttonStyles} onClick={handleSignIn}>
			...
			<span className='text-black hidden xs:inline'>Sign in with Steam</span>
		</Button>
	);
	if (isAuthenticated) {
		button = (
			<Button className={buttonStyles} onClick={handleSignOut}>
				...
				<span className='text-black hidden xs:inline'>Sign out</span>
			</Button>
		);
	}
	return (
		<header className='flex mt-5 flex-col gap-5 items-center'>
			<h1 className='text-center text-3xl font-bold xs:text-4xl md:text-5xl bg-(image:--background-gradient) bg-clip-text text-transparent'>
				Steam Game Tracker
			</h1>
			{button}
			<p className='text-xl text-center xs:hidden'>
				{isAuthenticated
